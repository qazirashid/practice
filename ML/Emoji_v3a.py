...
def pretrained_embedding_layer(word_to_vec_map, word_to_index):
    ...
    #word_to_index is a dictionary that contain each unique word in the vocab exactly once as a dictionary key. 
    # So the length of this dictionary equals the vocab size. We add 1 to account for UNKNOWN token.
    any_word = list(word_to_vec_map.keys())[0]
    #Select the first word in the word_to_vec_map dictionary. 
    emb_dim = word_to_vec_map[any_word].shape[0]    # define dimensionality of your GloVe word vectors (= 50)
    # We use the first word to get the size of the embedding vector. 
    ### START CODE HERE ###
    # Step 1
    # Initialize the embedding matrix as a numpy array of zeros.
    # See instructions above to choose the correct shape.
    emb_matrix = np.zeros((vocab_size, emb_dim))
    # Each unique word in the vocab corresponds to a row of emb_matrix
    # Each row of the emb_matrix is an embedding, which has as size of emb_dim.
    
    # Step 2
    # Set each row "idx" of the embedding matrix to be 
    # the word vector representation of the idx'th word of the vocabulary
    # word_to_index.items() gets all (key,value) pairs in the dictionary. where key is a word and the value is the index.
    
    for word, idx in word_to_index.items():
        emb_matrix[idx, :] = word_to_vec_map[word]
        #Select the idx row of the embedding matrix. This must be set to the embedding vector for word at indix idx.
...
def Emojify_V2(input_shape, word_to_vec_map, word_to_index):
    ...
    ### START CODE HERE ###
    #imported keras layers: Dense, Input, Dropout, LSTM, Activation
