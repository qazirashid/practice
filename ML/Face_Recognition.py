...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    pos_dist = tf.math.reduce_sum(tf.math.subtract(y_pred[0], y_pred[1]) * tf.math.subtract(y_pred[0], y_pred[1]), axis=-1) 
    # Step 2: Compute the (encoding) distance between the anchor and the negative
    neg_dist = tf.math.reduce_sum(tf.math.subtract(y_pred[0], y_pred[2]) * tf.math.subtract(y_pred[0], y_pred[2]), axis=-1)
    # Step 3: subtract the two previous distances and add alpha.
    basic_loss = tf.math.maximum( tf.math.subtract(pos_dist, neg_dist) + alpha, 0)
...
def img_to_encoding(image_path, model):
    ...
    # keras preprocessing module is used to resize the input image to a desired size of 160 x 160 pixels
    img = np.around(np.array(img) / 255.0, decimals=12)
    # Image is normalized so that max value in any channel does not exceed 1. The result is rounded to 12 decimal places
    x_train = np.expand_dims(img, axis=0)
    # A new axis is inserted at axis 0 (image number in the dataset)
    embedding = model.predict_on_batch(x_train)
    #The model is applied to the input -- a forward pass through the pre-trained model. An 1x128 embedding is obtained.
    return embedding / np.linalg.norm(embedding, ord=2)
    #Return the scaled embedding by dividing the embedding with its L2 norm. The returned vector will have a L2 norm = 1.
...
def who_is_it(image_path, database, model):
    ...
    encoding =  img_to_encoding(image_path, model)
    #The above function does several things as discussed above and repeated here. 
    # - it reads the image from the file.
    # - it preprocesses the image to a specific size 
    # - it applies a forward pass of the given model on the input preprocessed image
    # - it returns a 128 vector that is the encoding of the input image
    
    ## Step 2: Find the closest encoding ##
    
    # Initialize "min_dist" to a large value, say 100 (≈1 line)
    min_dist = 100
    #min_dist is set to 100 -- a large value as per the instructions
