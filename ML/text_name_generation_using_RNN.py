...
def sample(parameters, char_to_ix, seed):
    ...
    while (idx != newline_character and counter != 50):
        ...
        y = softmax(z)
        #print( x.shape, a.shape, z.shape, y.shape, Wax.shape, Waa.shape, Wya.shape, b.shape, by.shape)
...
def model(data_x, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        idx = j % len(examples)
        # Use loop index to build an index into list of examples. Since loop index can be much larger than list
        #of examples, we use modulo operator to cycle back when loop index exceeds the length of list of examples.
