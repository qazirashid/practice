...
def positional_encoding(positions, d):
    ...
    angle_rads = get_angles(pos_m, dims, d)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        self_mha_output = self.mha(x,x, attention_mask=mask, training=training )  # Self attention (batch_size, input_seq_len, fully_connected_dim)
        # skip connection
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out1, attn_weights_block1 = self.mha1(x, x, attention_mask=look_ahead_mask, training=training,  return_attention_scores=True)  # (batch_size, target_seq_len, d_model)
        ...
        mult_attn_out2, attn_weights_block2 = self.mha2(query= Q1, value=enc_output, attention_mask=padding_mask, training=training, return_attention_scores=True)  # (batch_size, target_seq_len, d_model)
...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, 
        ...
        for i in range(self.num_layers):
            ...
            #call parameters for decoderlayer: (x, enc_output, training, look_ahead_mask, padding_mask)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, input_sentence, output_sentence, training, enc_padding_mask, look_ahead_mask, dec_padding_mask):
        ...
        enc_output = self.encoder.call(x=input_sentence, training=training, mask=enc_padding_mask)  # (batch_size, inp_seq_len, fully_connected_dim)
        
        # call self.decoder with the appropriate arguments to get the decoder output
        # dec_output.shape == (batch_size, tar_seq_len, fully_connected_dim)
        dec_output, attention_weights = self.decoder.call(output_sentence, enc_output=enc_output, training=training, 
