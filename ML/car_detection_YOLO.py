...
def yolo_filter_boxes(boxes, box_confidence, box_class_probs, threshold = .6):
    ...
    # box_scores should have a shape of (19 x 19 x 5 x 80)
    # Step 2: Find the box_classes using the max box_scores, keep track of the corresponding score
    ##(≈ 2 lines)
    # IMPORTANT: set axis to -1
    box_classes = tf.math.argmax(box_scores, axis=-1)
    #the box_classes should have a shape of (19 x 19 x 5 ), where each element in an index into (19 x 19 x 5 x 80 ) tensor 
    # that ideces the max elment along the last axis. 
    # Note that the index in box_classes will identify the class or category that has the max score.
    #For example if the value of box_classes at (4, 3, 2) is 34, then it would mean that for the grid cell (4,3)
    # and anchor box 2 , the class with highest score is class 34. 
    box_class_scores = tf.math.reduce_max(box_scores, axis=-1)
    #box_class_scores will output a tensor of shape (19x19x5), where each element will be the score for each anchor box
    # in each grid cell. 
    # if the value at index (8,6,3) is 0.65, then it would mean that for grid cell (8,6), anchnor box 3, the maximum class 
    #class score is 0.64. Note that this tensor does not tell us which class has the max score. It only tells what is the 
    #the max score.
    ...
    # For each element in the 19x19x5 array, compare the value with 'threshold' and return 'True' or 'False'. Store the
    #result in a tensor of shape 19 x 19 x 5. 
    
    
    # Step 4: Apply the mask to box_class_scores, boxes and box_classes
    ## (≈ 3 lines)
    scores = tf.boolean_mask(box_class_scores, filtering_mask) 
    #input shape (19x19x5), mask shape (19x19x5), so output shape is (number of True mask entries x 1)
    boxes = tf.boolean_mask(boxes, filtering_mask)             
    #input share (19x19x5x4), mask shape (19x19x5), so output shape is (number of True mask entries x 4)
    classes = tf.boolean_mask(box_classes, filtering_mask)
    #input shape (19x19x5x1), mask shape (19x19x5) so soutput shape is (numbero of True mask entries x 1)
    ### END CODE HERE
...
    """
    ...
    ## (≈ 3 lines)
    box1_area = max(box1_x2 - box1_x1, 0) * max(box1_y2 - box1_y1,0)
    box2_area = max(box2_x2 - box2_x1, 0) * max(box2_y2 - box2_y1,0)
...
def yolo_eval(yolo_outputs, image_shape = (720, 1280), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    #The yolo outputs will be passed as an input argument in the form of a tuple of tensors as described above.
